...
export function MessagingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Add a useEffect to initialize the enhanced mock data

  useEffect(() => {
    // Initialize enhanced mock data
    try {
      // This is just for logging purposes
      console.log("Initializing enhanced mock data...")
      const channelCount = Object.keys(mockDataService.getChannels()).length
      const userCount = mockDataService.getUsers().length
      console.log(`Loaded ${channelCount} channels and ${userCount} users`)
    } catch (error) {
      console.error("Error initializing enhanced mock data:", error)
    }
  }, [])
  ...
  // Update the setActiveThread function to properly load thread messages
  ...
  const setActiveThread = useCallback(
    (messageId: string | null) => {
      ...
      if (messageId) {
        ...
        try {
          // Get thread messages from the mock data service
          const threadMsgs = mockDataService.getThreadMessages(messageId)
  ...
  const addMessageReaction = useCallback(
    async (messageId: string, reaction: string): Promise<void> => {
      ...
      try {
        ...
        // Update messages state with the new reaction
        setMessages((prevMessages) =>
          prevMessages.map((msg) => {
            if (msg.id === messageId) {
              // Check if user already reacted with this emoji
              const existingReactionIndex = msg.reactions.findIndex(
                (r) => r.userId === currentUser.id && r.reaction === reaction,
              )
    ...
    [currentUser, toast, activeThread],
  )
